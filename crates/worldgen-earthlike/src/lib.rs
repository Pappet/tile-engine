...
use worldgen_api::{Heightmap, StratigraphyMap, WorldgenConfig, WorldgenSet, subseed};
...
impl bevy_app::Plugin for EarthlikeWorldgenPlugin {
    fn build(&self, app: &mut bevy_app::App) {
        ...
        app.add_systems(
            bevy_app::PreStartup,
            (gen_heightmap, gen_stratigraphy, gen_chunks)
                .chain()
                .in_set(WorldgenSet::Maps),
        );
    }
}
...
mod tests {
    ...
    fn run_worldgen_with_seed(seed: u64) -> tile_core::world::World {
        ...
        app.world_mut()
            .remove_resource::<tile_core::world::World>()
            .unwrap()
    }
